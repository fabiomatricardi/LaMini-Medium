...
# ###########################################################################
#   Model_KR = MODEL FOR TRANSLATION ONLY FROM EN TO KR
#from transformers import AutoTokenizer, AutoModelForSeq2SeqLM
...
"""
with console.status("Preparing model and pipeline...", spinner="material"):   
    ...
    tokenizer_KR = AutoTokenizer.from_pretrained(Model_KR)  #google/byt5-small   #facebook/m2m100_418M
    console.print('[bold green] Inizialize AI model...')
    #model_ttKR = AutoModelForSeq2SeqLM.from_pretrained(Model_KR, from_tf=True)  #Helsinki-NLP/opus-mt-en-it  or #Helsinki-NLP/opus-mt-it-en
    model_ttKR = AutoModelForSeq2SeqLM.from_pretrained(Model_KR)
    TToKR = pipeline("translation", model=Model_KR, tokenizer=tokenizer_KR)
...
with console.status("Translation to Korean in progress...", spinner="pong"):
  for item in texts:
    line = TToKR(item.page_content)[0]['translation_text']
